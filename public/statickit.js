  ...
  */
  ...
  function Element() {
    ...
    this._setProperty = function(arr, obj, key, val) {
      ...
      arr.push(
        'function' === typeof obj
          ? new obj(key.toLowerCase(), String(val))
          : obj
      );
    };
      ...
      for (var i = 0, len = childNodes.length; i < len; i++) {
        ...
        if (child === existingChild) {
          i === 0
            ...
            : childNodes.splice(i, 0, newChild);
          break;
        }
      }
    }
  ...
  Text.prototype.replaceData = function(offset, length, str) {
    this.value =
      this.value.slice(0, offset) + str + this.value.slice(offset + length);
  };
  ...
  */
  ...
  var renderErrors = function renderErrors(config, errors) {
    ...
    Array.from(elements).forEach(function(element) {
      ...
      var fullMessage =
        errorMessages[code] || ''.concat(prefix, ' ').concat(error.message);
  ...
  var init = function init(client, props) {
    ...
    if (!props.element)
      throw new Error('You must define an `element` property');
    var form = getFormElement(props.element);
    if (!form)
      throw new Error('Element `'.concat(props.element, '` not found'));
